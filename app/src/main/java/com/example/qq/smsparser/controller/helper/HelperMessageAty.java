...
public class HelperMessageAty extends Activity implements View.OnClickListener{
    ...
    private LineChartView saleLineChartView;
    private LineChartView profitLineChartView;
    ...
    private void initView(){
        ...
        textView= (TextView) findViewById(R.id.tv_aty_helper_message);
        mLineChartView= (LineChartView) findViewById(R.id.lvc_aty_helper_send);
        saleLineChartView= (LineChartView) findViewById(R.id.lvc_aty_helper_sale);
        profitLineChartView= (LineChartView) findViewById(R.id.lvc_aty_helper_profit);
    ...
    private void initTestData(){
        datas.add(getHashMap("发货数量"));
        datas.add(getHashMap("770"));
        datas.add(getHashMap("发货销售额"));
        datas.add(getHashMap("500"));
        datas.add(getHashMap("快递费用"));
        datas.add(getHashMap("100"));
        datas.add(getHashMap("需要支付的工资"));
        datas.add(getHashMap("4000"));
        datas.add(getHashMap("其他报销费用"));
        datas.add(getHashMap("200"));
        datas.add(getHashMap("帮忙产生的利润"));
    ...
    private void setLinesDatas() {
        ...
        */
        ...
        saleLineChartView.setLineChartData(mLineData);    //设置图表控件
        profitLineChartView.setLineChartData(mLineData);    //设置图表控件
    }
    ...
    private void resetViewport() {
        ...
        final Viewport v1 = new Viewport(saleLineChartView.getMaximumViewport());
        v1.left = 0;                             //坐标原点在左下
        v1.bottom = 0;
        v1.top = 100;                            //最高点为100
        v1.right = numberOfPoints - 1;           //右边为点 坐标从0开始 点号从1 需要 -1
        saleLineChartView.setMaximumViewport(v1);   //给最大的视图设置 相当于原图
        saleLineChartView.setCurrentViewport(v1);   //给当前的视图设置 相当于当前展示的图

        final Viewport v2 = new Viewport(profitLineChartView.getMaximumViewport());
        v2.left = 0;                             //坐标原点在左下
        v2.bottom = 0;
        v2.top = 100;                            //最高点为100
        v2.right = numberOfPoints - 1;           //右边为点 坐标从0开始 点号从1 需要 -1
        profitLineChartView.setMaximumViewport(v2);   //给最大的视图设置 相当于原图
        profitLineChartView.setCurrentViewport(v2);   //给当前的视图设置 相当于当前展示的图
